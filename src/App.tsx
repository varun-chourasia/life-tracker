...
const DEFAULT_ROUTINE: RoutineItem[] = [
  ...
  },
  {
    ...
    startTime: "05:00",
    endTime: "07:00",
    activity: "Deep Work 1 (Math/Stats)",
    category: "Focus",
  },
  {
    id: "r3",
...
const ROUTINE_COLORS: Record<RoutineCategory, string> = {
  ...
  Work: "text-blue-600 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400",
};
...
const BADGES: Badge[] = [
  {
    ...
    isUnlocked: (tasks) => tasks.some((t) => t.completed),
  },
  {
    id: "early_bird",
    name: "Early Bird",
    ...
    isUnlocked: (tasks) =>
      tasks.some((t) => t.completed && parseInt(t.startTime.split(":")[0]) < 8),
  },
  {
    id: "machine",
    name: "The Machine",
  ...
  },
  {
    ...
    isUnlocked: (tasks) =>
      tasks.some(
        (t) => t.completed && parseInt(t.startTime.split(":")[0]) >= 20
      ),
  },
];
...
function TrackerApp() {
        ...
        if (currentTimeVal > endH * 60 + endM) {
          try {
            new Notification("⚠️ Task Overdue!", {
              body: `Mark '${task.title}' as complete?`,
  ...
  const handleResetStats = async () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to reset your tasks and XP? Your profile and routine will be kept."
      )
    ) {
  ...
  const getHeatmapColor = (count: number) => {
    ...
    if (count <= 2)
      return "bg-green-900 text-green-300 font-bold border border-green-700";
    if (count <= 4)
      return "bg-green-700 text-white font-bold border border-green-500";
  ...
  return (
    <div className="flex h-screen bg-slate-900 font-sans text-white dark">
      ...
      <button
        ...
        className="md:hidden fixed top-4 right-4 z-40 bg-slate-800 p-2 rounded-lg shadow-md text-white"
      >
        <Menu size={24} />
      </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-6 rounded-xl shadow-sm border bg-slate-800 border-slate-700">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
                      <div
                        key={d}
                        className="text-xs text-center text-slate-500 font-bold"
                      >
                        {d}
                      </div>
                    ))}
                    {monthGrid.map((day, i) =>
                      day ? (
                        <div
                          key={i}
                          className={`w-full aspect-square rounded-sm flex items-center justify-center text-[10px] font-bold transition-all ${getHeatmapColor(
                            day.count
                          )}`}
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-xl shadow-sm border bg-slate-800 border-slate-700 flex flex-col items-center justify-center">
                    <h3 className="text-sm font-semibold mb-2 w-full border-b pb-2 border-slate-700 text-white">
                      Today's Focus
                    </h3>
                    <div className="relative w-32 h-32 flex items-center justify-center">
                      <svg className="w-full h-full transform -rotate-90">
                        ...
                        <circle
                          ...
                          strokeDasharray={314}
                          strokeDashoffset={
                            314 - (314 * routineStats.percent) / 100
                          }
                          strokeLinecap="round"
                        />
                      </svg>
                ...
                todaysTasks.map((t) => (
                  <div
                    ...
                    className={`p-4 rounded-xl shadow-sm border flex items-center gap-4 ${
                      activeTaskId === t.id
                        ? "ring-2 ring-blue-500 bg-blue-900/20 border-blue-800"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-slate-500 mt-1">
                        ...
                        {activeTaskId === t.id && (
                          <span className="text-blue-400 font-bold bg-blue-900/50 px-2 py-0.5 rounded-full flex items-center gap-1">
